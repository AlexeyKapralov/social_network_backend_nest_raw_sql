...
import { PostWithLikesType } from '../domain/posts.entity';
...
export class PostsQueryRepository {
    ...
    async findPosts(query: QueryDtoBase, userId: string = '00000000-0000-0000-0000-000000000000') {
        ...
        const uniquePosts = Array.from(uniquePostsSet).slice(offset, offset + limit)
        ...
        uniquePosts.forEach(postId => {
            ...
            const postSource: PostWithLikesType[] = postsSourceWithNewestLikes
                .filter((p) => p.id === postId);

            postSource.map((p: PostWithLikesType) => {
                if (p.login !== null && p.userId !== null && p.addedAt !== null) {
                    if (likes.length < 3) {
                        likes.push(
                            {
                                addedAt: p.addedAt,
